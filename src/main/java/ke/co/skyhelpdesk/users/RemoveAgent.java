...
public class RemoveAgent implements HttpHandler {
            ...
            if (!checkAccessTokenValidity(httpServerExchange)) {
                ...
                if (queryData != null) {
                    if (queryData.containsValue("")) {
                        ...
                        if (!results.get(0).get("application_user_group_id").equals(3)) {
                            response.put("status", "error");
                            response.put("message", "That user is not an agent.You can not delete a user who is not an agent.");
                            APIResponses.sendResponses(httpServerExchange, response, StatusCodes.BAD_REQUEST);
                        }else {
//                     queryHandler.add(sqlQueryThree,queryData);
                            queryHandler.add(sqlQuery, queryData);
                            response.put("status", "SUCCESS");
                            response.put("message", "The agent  with email as " + results.get(0).get("email") + " has been removed from the system.");
