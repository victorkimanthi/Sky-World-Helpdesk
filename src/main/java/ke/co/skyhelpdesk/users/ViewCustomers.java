...
public class ViewCustomers implements HttpHandler {
    public void handleRequest(HttpServerExchange httpServerExchange) throws Exception {
        String sqlQuery="select user_account.* from user_account join user_account_application_user_group on user_account.user_account_id = user_account_application_user_group.user_account_id where application_user_group_id = 2 ORDER BY date_created DESC LIMIT 10 OFFSET :OFFSET";
        ...
        try {
            if (!checkAccessTokenValidity(httpServerExchange)) {
                ...
                resultsMap.put("message", "The list of customers has been retrieved successfully");
