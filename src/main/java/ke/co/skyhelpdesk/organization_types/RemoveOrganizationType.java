...
public class RemoveOrganizationType implements HttpHandler {
    public void handleRequest(HttpServerExchange httpServerExchange) {
        ...
        String sqlQuery = "select  organization_type_name from organization_type WHERE organization_type_id=(:organization_type_id)";
        String sqlQueryThree = "DELETE FROM  organization_type WHERE organization_type_id=(:organization_type_id)";
...
//            } else {
                ...
                queryData.put("organization_type_id", parameters.get("id").get(0));
                if (queryData != null) {
                    if (queryData.containsValue("")) {
                        ...
                        try {
                            ...
                            if(results.isEmpty()){
                                response.put("status", "ERROR");
                                response.put("error", "The organization type  does not exist.");
                                APIResponses.sendResponses(httpServerExchange, response, StatusCodes.BAD_REQUEST);
                            }else {
                                queryHandler.add(sqlQueryThree, queryData);
                                response.put("status", "SUCCESS");
                                response.put("message", results.get(0).get("organization_type_name") + " has been removed  as an organization type.");
